...
function Library:CreateWindow(WindowInfo)
    ...
    if WindowInfo and WindowInfo.LiquidGlass then
        task.spawn(function()
            local MainFrame = nil
            for i = 1, 100 do
                if typeof(Window) == "table" and (Window.MainFrame or Window.Frame) then
                    MainFrame = Window.MainFrame or Window.Frame
                    break
                end
                task.wait(0.1)
            end
            ...
            pcall(function()
                local GlassVariant = MaterialService:FindFirstChild("LiquidGlass") or Instance.new("MaterialVariant")
                GlassVariant.Name = "LiquidGlass"
                GlassVariant.BaseMaterial = Enum.Material.Glass
                GlassVariant.Parent = MaterialService
            end)

            local GlassPart = Instance.new("Part")
            GlassPart.Name = "Voidhub_LiquidGlass"
            GlassPart.Material = Enum.Material.Glass
            GlassPart.Transparency = 0.45
            GlassPart.Color = Color3.fromRGB(20, 20, 20)
            ...
            local function UpdateGlass()
                if not MainFrame or not MainFrame.Parent or not MainFrame.Visible or not Library.Toggled then 
                ...
                local TopLeftRay = Camera:ScreenPointToRay(Position.X, Position.Y)
                local BottomRightRay = Camera:ScreenPointToRay(Position.X + Size.X, Position.Y + Size.Y)
                
                local WorldPos = TopLeftRay.Origin + TopLeftRay.Direction * Depth
                local WorldSizeX = (BottomRightRay.Origin + BottomRightRay.Direction * Depth - WorldPos).X
                local WorldSizeY = (BottomRightRay.Origin + BottomRightRay.Direction * Depth - WorldPos).Y
                
                GlassPart.Size = Vector3.new(math.abs(WorldSizeX) * 21, math.abs(WorldSizeY) * 21, 0.01)
                GlassPart.CFrame = Camera.CFrame * CFrame.new(0, 0, -Depth)
                
                GlassPart.Transparency = 0.45
            end
            
            game:GetService("RunService").RenderStepped:Connect(UpdateGlass)
    ...
    ToggleBtnCorner.CornerRadius = UDim.new(0, Library.CornerRadius)
    ToggleBtnCorner.Parent = ToggleButton.Button
    ...
    LockBtnCorner.CornerRadius = UDim.new(0, Library.CornerRadius)
    LockBtnCorner.Parent = LockButton.Button
...
function Library:CreateLoading(LoadingInfo)
    ...
    table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = SliderBar }))
    ...
    table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = SliderFill }))
