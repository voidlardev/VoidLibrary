...
function Library:CreateWindow(WindowInfo)
    ...
    task.spawn(function()
        ...
        if WindowInfo.BackgroundTransparency and WindowInfo.BackgroundTransparency > 0 then
            MainFrame.BackgroundTransparency = WindowInfo.BackgroundTransparency
        end
        
        if WindowInfo.BackgroundImage and WindowInfo.BackgroundImage ~= "" then
            local BgImage = Instance.new("ImageLabel")
            BgImage.Name = "GSHUB_CustomBackground"
            BgImage.Size = UDim2.new(1, 0, 1, 0)
            BgImage.Position = UDim2.new(0, 0, 0, 0)
            BgImage.Image = WindowInfo.BackgroundImage
            BgImage.BackgroundTransparency = 1
            BgImage.ZIndex = MainFrame.ZIndex - 1
            BgImage.Parent = MainFrame
            
            local Corner = MainFrame:FindFirstChildOfClass("UICorner")
            if Corner then
                Corner:Clone().Parent = BgImage
            end
        end
    end)
